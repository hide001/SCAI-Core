...
This project aims to provide installation, running, and maintenance capabilities of **SCAI validator node** for potential and existing SCAI backers. This repository has multiple release candidates inline so we recommend checking for updates for better functions and stability.
...
**Note regarding use of GPUs -** GPUs are primarily used in POW consensus chains. Being a DPos SCAI has not only more TPS and fast block production but also doesn't need a GPU altogether for its purpose.
...
* Once "unauthorized validators" warning shows up, go to https://validator.securechain.ai and click "Become a validator". Fill in all the details in the form, in the "Fee address" field enter the validator wallet address that you imported into your metamask. Proceed further
...
To install the SCAI validator node in ubuntu linux
```bash
  sudo -i
...
```bash
  sudo -i
  git clone https://github.com/SecureChainAI/SCAI-Core.git
  cd core-blockchain
