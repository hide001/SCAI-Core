...
task4(){
  ...
  if [[ $totalValidator -gt 0 ]]; then
      ...
      echo -e '\nbash /root/SCAI-Core/node-start.sh --validator' >>/etc/profile
  fi
...
task8(){
  ...
  while [[ $i -le $totalValidator ]]; do
    ...
    ./bin/geth --datadir ./chaindata/node$i account new --password ./chaindata/node$i/pass.txt
    ((i += 1))
  done
...
createRpc(){
  ...
  while [[ $i -le $totalNodes ]]; do
    ...
    echo -e "\nVHOST=$vhost" >> ./.env
    ./bin/geth --datadir ./chaindata/node$i init ./genesis.json
    ((i += 1))
  done
}
